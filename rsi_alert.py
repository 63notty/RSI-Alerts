...
1. TradingView (via tradingview-ta) for crypto, forex, and gold — pulled
   ...
   work (Binance, OANDA, Forex.com). This matches TradingView's own chart
   values exactly.

2. Yahoo Finance for Nasdaq 100, S&P 500, US Oil, and Silver — TradingView's
   API has a hard limitation where it does NOT support index-type
   instruments at all (confirmed directly from the library's own
   documentation), regardless of which broker exchange is used. For these,
   we pull futures price history directly and calculate RSI ourselves using
   Wilder's smoothing method — the same standard formula TradingView uses
   internally.

Note on UK 100: it uses the plain cash FTSE index, since no reliable free
continuous futures ticker exists for it. This means its RSI may be
unreliable outside London trading hours (8 AM-4:30 PM UK time), when the
cash index goes stale but broker CFDs keep moving.
...
# ---- Source 1: TradingView (crypto, forex, gold) — confirmed working exchanges ----
TV_WATCHLIST = [
...
# ---- Source 2: Yahoo Finance (indices/commodities TradingView's API can't serve) ----
YF_WATCHLIST = [
    {"name": "US OIL",     "ticker": "CL=F"},   # WTI Crude futures — same feed brokers use
    {"name": "NASDAQ 100", "ticker": "NQ=F"},   # futures — cash index goes stale overnight
    {"name": "US 500",     "ticker": "ES=F"},   # futures — cash index goes stale overnight
    {"name": "XAG/USD",    "ticker": "SI=F"},   # Silver futures — same feed brokers use
    {"name": "UK 100",     "ticker": "^FTSE"},  # cash index — unreliable outside London hours
]
